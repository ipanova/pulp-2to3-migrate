...
class DockerContentMigrationFirstStage(ContentMigrationFirstStage):
    ...
    async def migrate_to_pulp3(self, batch, pb=None):
        ...
        for pulp2content in batch:
            ...
            if pulp_2to3_detail_content.type == 'docker_tag':
                dc = DeclarativeContent(content=pulp3content)
                # TODO find artifact from tagged_manifest and use in DA
                #m=Manifest.objects.filter(digest=pulp_2to3_detail_content.tagged_manifest).first()
                #artifact = m._artifacts.get()
                # save here? 
                #pulp3content.tagged_manifest = m
                #pulp3content.save()
            else:
                artifact = await self.create_artifact(pulp2content.pulp2_storage_path,
                                                      pulp_2to3_detail_content.expected_digests,
    ...
    async def run(self):
        ...
        async for dc in self.items():
            ...
            if dc.extra_data.get('relation'):
                ...
                self.relate_blob(dc)
            elif dc.extra_data.get('config_blob_rel'):
